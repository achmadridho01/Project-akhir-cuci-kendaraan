...
@section('title', 'Harga Paket Cuci')

@section('content')
<div class="container mt-4">

    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4>Harga Paket Cuci</h4>
        <a href="{{ route('admin.paketharga.create') }}" class="btn btn-primary">
             Tambah Harga
        </a>
    </div>
            ...
            <table class="table table-bordered mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $h->paketCuci->nama_paket }}</td>
                        <td>{{ $h->tipeKendaraan->nama_tipe }}</td>
                        <td>Rp {{ number_format($h->harga,0,',','.') }}</td>
                        <td>
                            <a href="{{ route('admin.paketharga.edit',$h->id) }}"
                               class="btn btn-warning btn-sm">Edit</a>

                            <form method="POST" action="{{ route('admin.paketharga.destroy', $h->id) }}" class="d-inline delete-form">
                                @csrf
                                @method('DELETE')
                    ...
                    <tr>
                        <td colspan="5" class="text-center text-muted">
                            Data harga paket cuci belum ada
                        </td>
                    </tr>
                    @endforelse
                </tbody>
